...
function Testing() {
  ...
  //   try {
  //     const response = await API.post('testing', '/data', { body: { data } });
  ...
  const handleDiseaseCheckboxChange = (disease) => {
    ...
    sendDataToAPI({ disease: disease, checked: !diseaseCheckboxes[disease] });
  };
...
useEffect(() => {
  console.log('Checked Disease Boxes:', Object.keys(diseaseCheckboxes).filter(key => diseaseCheckboxes[key]));
  console.log('Checked Sex Boxes:', Object.keys(sexCheckboxes).filter(key => sexCheckboxes[key]));
  console.log('Checked inCITE Top 25% Boxes:', Object.keys(inCITETop25Checkboxes).filter(key => inCITETop25Checkboxes[key]));
  console.log('Checked inCITE Bottom 25% Boxes:', Object.keys(inCITEBottom25Checkboxes).filter(key => inCITEBottom25Checkboxes[key]));
  console.log('Checked Cell Type Boxes:', Object.keys(cellTypeCheckboxes).filter(key => cellTypeCheckboxes[key]));
  console.log('Checked Preperation Type Boxes:', Object.keys(preperationTypeCheckboxes).filter(key => preperationTypeCheckboxes[key]));
  console.log('Checked Filtered inCITE Top 25% Boxes:', Object.keys(filterTop25Checkboxes).filter(key => filterTop25Checkboxes[key]));
  console.log('Checked Filtered inCITE Bottom 25% Boxes:', Object.keys(filterBottom25Checkboxes).filter(key => filterBottom25Checkboxes[key]));
  console.log('Checked Filtered Cell Type Boxes:', Object.keys(filtercellTypeCheckboxes).filter(key => filtercellTypeCheckboxes[key]));
  console.log('Checked Filtered Batch Boxes:', Object.keys(filterBatchCheckboxes).filter(key => filterBatchCheckboxes[key]));
